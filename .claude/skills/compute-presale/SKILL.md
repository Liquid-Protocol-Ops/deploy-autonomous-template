...
**Canonical contract (policy decision 2026-06-12): `LiquidPresaleVault`** — source `liquid-website/contracts/presale/src/LiquidPresaleVault.sol`, deployable bytecode embedded in `liquid-website/src/lib/presale.ts` (`buildVaultInitCode`). The older `MintDiemPresaleVault`, `ComputePresaleVault`, and `StakesaleVault` are **superseded — do not deploy them**. `scripts/deploy-compute-presale.ts` still targets the old bytecode; do not use it until retargeted (blocked on bytecode provenance).
...
```bash
cd <liquid-protocol-v0 checkout>
...
```
Safe-signed by the protocol multisig; operator-only step.
...
End-to-end creator/depositor guide with FAQ: `docs/PRESALE_GUIDE.md` (this repo). Policy + reconcile record: `docs/superpowers/plans/2026-06-12-presale-policy-reconcile.md`.
