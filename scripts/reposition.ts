...
async function main() {
  ...
  const wethBal = wethStart + extractTransferAmount(collectLogs, ADDRESSES.WETH, agentAddress);
  const diemBal = diemStart + extractTransferAmount(collectLogs, ADDRESSES.DIEM, agentAddress) + claimedDiem;
  ...
  if (skipSwap) {
    ...
    if (belowRange) {
      ...
      diemFromSwap = extractTransferAmount(swapReceipt.logs, ADDRESSES.DIEM, agentAddress);
    } else {
      diemSpent   = swapAmountIn;
      wethFromSwap = extractTransferAmount(swapReceipt.logs, ADDRESSES.WETH, agentAddress);
    }
  } // end swap block
