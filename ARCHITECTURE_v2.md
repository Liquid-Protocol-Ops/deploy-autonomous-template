...
The remaining `CLAUDE.md` "explicitly confirmed" decisions (self-modification scope, threshold-triggered deployment, public-repo-per-agent, holder-suggestion thresholds, scheduled run (GitHub Actions) cadence, orchestrator quorum) are **not** affected.
...
- **`scheduler`** — unchanged in shape; the scheduled run (GitHub Actions) substrate is the v0 substrate to be replaced by a TEE substrate post-MVP.
