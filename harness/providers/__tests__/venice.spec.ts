...
afterEach(() => {
  ...
  delete process.env['VVV_TOKEN_ADDRESS'];
  delete process.env['MIN_DIEM_WEI'];
  vi.clearAllMocks();
});
...
describe('loadConfig', () => {
  it('loads required env vars', () => {
    ...
    expect(cfg.vvvStakingAddress).toBe(DUMMY_ADDRESS);
  ...
  it('defaults vvvStakingAddress to the known sVVV contract when both env vars are missing', () => {
    delete process.env['VVV_STAKING_ADDRESS'];
    delete process.env['VENICE_STAKING_ADDRESS'];
    const cfg = loadConfig();
    expect(cfg.vvvStakingAddress.toLowerCase()).toBe(
      '0x321b7ff75154472b18edb199033ff4d116f340ff',
    );
  });

  it('defaults vvvAddress to the known VVV ERC-20 when VVV_TOKEN_ADDRESS is unset', () => {
    delete process.env['VVV_TOKEN_ADDRESS'];
    const cfg = loadConfig();
    expect(cfg.vvvAddress.toLowerCase()).toBe(
      '0xacfe6019ed1a7dc6f7b508c02d1b04ec88cc21bf',
    );
  });

  it('defaults minDiemWei when MIN_DIEM_WEI is unset', () => {
    delete process.env['MIN_DIEM_WEI'];
    const cfg = loadConfig();
    expect(cfg.minDiemWei).toBe(parseEther('0.01'));
  });
