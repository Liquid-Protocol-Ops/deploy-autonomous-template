...
describe('makeTxSenderFromPrivy', () => {
  ...
  it('sends eth_sendTransaction to an allowlisted destination and returns the tx hash', async () => {
    const mockFetch = vi.fn().mockResolvedValueOnce({
      ok: true,
    ...
    const hash = await txSender({
      to: ADDRESSES.NFPM_V3, // a known protocol contract — passes the destination allowlist
      data: '0xabcd',
    });
    ...
    expect(body.params.transaction.to).toBe(ADDRESSES.NFPM_V3);
    expect(body.params.transaction.data).toBe('0xabcd');
  });

  it('rejects a non-allowlisted destination before any fetch (fail closed)', async () => {
    const mockFetch = vi.fn();
    const txSender = makeTxSenderFromPrivy(cfg, mockFetch as unknown as typeof fetch);
    await expect(
      txSender({ to: '0xBADbADbadBADBaDBAdbAdbADBaDbadBaDBaDbAD0', data: '0xabcd' }),
    ).rejects.toThrow(/TxDestinationNotAllowed/);
    expect(mockFetch).not.toHaveBeenCalled();
  });

  it('throws on non-ok send-transaction response', async () => {
    const mockFetch = vi.fn().mockResolvedValueOnce({ ok: false, status: 500, text: async () => 'Internal Server Error' });
    const txSender = makeTxSenderFromPrivy(cfg, mockFetch as unknown as typeof fetch);
    await expect(txSender({ to: ADDRESSES.FEE_LOCKER, data: '0x' }))
