...
import { guardTxSender, type GuardOptions } from './tx-allowlist.js';
...
// Returns the transaction hash. `value` is optional (most harness calls are
// zero-value contract calls); the destination-allowlist guard reads it for the
// optional value cap.
export type TxSender = (params: { to: Address; data: Hex; value?: bigint }) => Promise<Hex>;
...
export function makeTxSenderFromEnv(rpcUrl: string, guardOpts: GuardOptions = {}): TxSender {
  const raw = process.env[AGENT_PRIVATE_KEY];
  ...
  // Destination-allowlist + optional value cap (fail closed) before signing.
  return guardTxSender(
    ({ to, data, value }) => walletClient.sendTransaction({ to: to!, data, value }),
    account.address,
    guardOpts,
  );
}
...
// `selfAddress` is the agent's own wallet address — it joins the destination
// allowlist (alongside the ADDRESSES constants). Optional: pass it whenever the
// address is known (the tick resolves it via loadSignerFromPrivy). When omitted
// it is `undefined` and simply NOT allow-listed — never a placeholder like the
// zero address, which would otherwise permit burn-to-zero sends (review note #47).
export function makeTxSenderFromPrivy(
  config: PrivyWalletConfig,
  fetchFn: typeof fetch = fetch,
  selfAddress: Address | undefined = undefined,
  guardOpts: GuardOptions = {},
): TxSender {
  const send = async ({ to, data, value }: { to?: Address; data: Hex; value?: bigint }): Promise<Hex> => {
    const transaction: Record<string, unknown> = { to, data };
  ...
  // Destination-allowlist + optional value cap (fail closed) before signing.
  return guardTxSender(send, selfAddress, guardOpts);
}
